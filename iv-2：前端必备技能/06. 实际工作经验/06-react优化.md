...
- 要彻底理解 shouldComponentUpdate 及其周边
- 在 React 的世界中，要让 `不可变数据` 深入骨髓
...
JS 关键词的冲突：JSX 中 `for` 写成 `htmlFor` ， `class` 写成 `className`
...
setState 是异步更新的，要在 callback 中拿到最新的 state 值
