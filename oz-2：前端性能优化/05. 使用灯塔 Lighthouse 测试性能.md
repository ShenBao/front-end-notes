...
（2）**预连接所要请求的源**，提前建⽴与所要访问资源之间的⽹络连接，或者加快域名的解析速度都能有效地提⾼⻚⾯的访问性能。这⾥给出了两种⽅案：⼀种是设置〈link rel="preconnect"〉的预连接，另⼀种是设置〈link rel="dns-prefetch"〉的 DNS 预解析。
...
Gzip、Deflate 和 Brotli 都是常用的数据压缩算法，它们在 Web 开发中用于减少传输数据的大小，从而提高页面加载速度和性能。以下是对这三种压缩算法的简要介绍：

Gzip：
- Gzip 是一种广泛使用的压缩算法，特别是在 HTTP 协议中。
- 它使用 DEFLATE 算法进行压缩，DEFLATE 是 LZ77 算法和 Huffman 编码的组合。
- Gzip 压缩效率较高，适用于大多数文本和二进制数据。

Deflate：
- Deflate 是一种无损数据压缩算法，它结合了 LZ77 算法和 Huffman 编码。
- 与 Gzip 不同，Deflate 不包含额外的元数据，因此压缩后的数据更小。
- Deflate 常用于 HTTP 响应头中的 Content-Encoding 字段。

Brotli：
- Brotli 是一种相对较新的压缩算法，由 Google 开发。
- 它旨在提供比 Gzip 和 Deflate 更高的压缩比，特别是在处理文本数据时。
- Brotli 使用 LZ77 算法、Huffman 编码和二阶上下文混合算法（Second-Order Context Mixing）。
- Brotli 在现代浏览器中得到了广泛支持，但在一些旧版浏览器中可能不受支持。

在实际应用中，选择哪种压缩算法取决于具体的需求和环境：
- 兼容性：Gzip 和 Deflate 在大多数浏览器和服务器中都有很好的支持，而 Brotli 在一些旧版浏览器中可能不受支持。
- 压缩效率：Brotli 通常提供更高的压缩比，特别是在处理文本数据时。如果目标用户群体主要使用支持 Brotli 的现代浏览器，可以考虑使用 Brotli。
- 性能：Brotli 的解压缩速度可能比 Gzip 和 Deflate 慢，因此在某些情况下，Gzip 或 Deflate 可能是更好的选择。

在 Web 开发中，可以通过配置服务器来选择使用哪种压缩算法。例如，在 Nginx 中，可以通过以下配置来启用 Gzip 和 Brotli 压缩：
...
通过合理选择和配置压缩算法，可以显著提高网站的加载速度和用户体验。
