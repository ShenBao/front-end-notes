...
基于 React 实现一个可复用的展示鼠标位置的高阶组件（HOC），要求接收一个组件，返回一个新的组件。
...
将类似以下 JSON 表示的树状结构（可以无限层级）通过 parseDOM 函数（使用 document.createElement，document.createTextNode，appendChild 等方法）生成一颗 DOM 树（返回一个 element 元素）
